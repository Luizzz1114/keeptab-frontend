...
<template>
  ...
  >
    ...
    >
      <div class="flex items-center gap-3">
        <div
          class="flex size-7 items-center justify-center rounded-lg bg-white/20 text-sm font-extrabold text-white ring-1 ring-white/30"
        >
          {{ cart.reduce((acc, item) => acc + item.cantidad, 0) }}
        </div>
        <span class="font-bold tracking-wide text-white">Ver Carrito</span>
      </div>
      <span class="mr-1 text-xl font-black tracking-tight text-white">
  ...
  >
    <div
      class="flex h-16 shrink-0 items-center justify-between border-b border-dashed border-zinc-200 p-4 dark:border-zinc-700"
    >
      <div class="flex items-center gap-3">
        ...
        <span
          class="rounded-full border border-zinc-200 bg-zinc-100 px-3 py-1 text-xs font-bold text-zinc-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300"
        >
          {{ cart.length }} items
        </span>
      </div>
    ...
    <div class="flex flex-1 flex-col gap-3 overflow-y-auto p-3">
      ...
      >
        <div class="mb-3 flex items-start justify-between">
          <div>
            <h3 class="text-sm font-bold text-zinc-700 dark:text-zinc-100">
          ...
          <span class="text-sm font-black text-zinc-700 dark:text-white">{{
            formatCurrency(item.producto.precio * item.cantidad)
          }}</span>
        </div>
        <div class="flex items-center justify-between">
          <div
            class="flex items-center gap-1 rounded-xl border border-zinc-200 bg-zinc-50 p-1 dark:border-zinc-700 dark:bg-zinc-900"
          >
            <Button
    ...
    <div
      class="flex shrink-0 flex-col gap-4 border-t border-dashed border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900"
    >
      ...
      <div class="flex items-end justify-between">
        ...
        <span class="text-xl font-black tracking-tight text-zinc-700 dark:text-white">{{
